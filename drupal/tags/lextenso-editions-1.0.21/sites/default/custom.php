...
function custom_url_rewrite_outbound(&$path, &$options, $original_path) {
  ...
  // /eja/documents/publisher/* -> /ouvrages/documents/editeur/*
  // /eja/* -> /ouvrages/*
  if (preg_match('|^eja/publisher(/.*)|', $path, $matches)) {
    $path = 'ouvrages/editeur' . $matches[1];
  } elseif (preg_match('|^eja/documents/publisher(/.*)|', $path, $matches)) {
    $path = 'ouvrages/documents/editeur' . $matches[1];
...
function custom_url_rewrite_inbound(&$result, $path, $path_language) {
  ...
  if (preg_match('|^ouvrages/editeur(/.*)|', $result, $matches)) {
    ...
    $modified = true;
  } elseif (preg_match('|^ouvrages/documents/editeur(/.*)|', $result, $matches)) {
    $result = 'eja/documents/publisher' . $matches[1];
    $modified = true;
